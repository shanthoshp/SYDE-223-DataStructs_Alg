...
protected:
  ...
  //HELPER FUNCTIONS
  void deleteAllNodes(TaskItem* node);
  bool existsCheck(TaskItem* node, int val) const;
  unsigned int getHeight(TaskItem* node) const;
  void PreOrder(TaskItem* node) const;
  void insertVal(TaskItem* node, TaskItem* val);
