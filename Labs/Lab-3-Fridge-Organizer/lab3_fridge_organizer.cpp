...
bool FridgeOrganizer::add_meal_portion(string n_name, string n_expiry) {
  ...
  FridgeOrganizer::MealPortion* newMeal = new MealPortion(n_name, n_expiry);
  ...
  // step4 else if there is space for a new stack
      ...
      stack<MealPortion*>* temp = new stack<MealPortion*>;
...
FridgeOrganizer::MealPortion FridgeOrganizer::remove_meal_portion() {
  ...
  FridgeOrganizer::MealPortion m = MealPortion("EMPTY", "N/A");
  ...
  if (stacks.empty() || cur_stack -> size() == 0) {
	...
	return MealPortion(m.name, m.expiry);
}
